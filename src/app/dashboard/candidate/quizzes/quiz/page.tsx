...
function Page() {
  ...
  const [correctAnswersObj, setCorrectAnswersObj] = useState({});
  const [getScoreBackendCall, setGetScoreBackendCall] = useState(true);
  ...
  const handleSubmit = useCallback(
    async (userAnswers: any) => {
      ...
      const payLoad = {
        ...
        score: `${correctResults.length}/${questions.length}`,
        questionsIds: correctResults.map(item=> item._id)
      };
      try {
        ...
        console.log(`${correctResults.length}/${questions.length}`);
        ...
        if (response.status !== 200) {
          ...
          setGetScoreBackendCall(false); 
            ...
            //   }/${questions.length}&questions=${JSON.stringify(correctResults)}`
            // );
        }
      } catch (error) {
        setAlert({
